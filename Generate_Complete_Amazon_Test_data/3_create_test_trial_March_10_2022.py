        ...
        if self.unknown_writer_novel_dict is not None:
            while unknown_writer_not_seen_before not in self.unknown_writer_novel_dict:
                ...
                if tmp_index > 500:
                    sys.exit(">>>>>>>>>> ERROR <<<<<<<<<<< scenario_7_with_novelty_multiple")
                unknown_writer_not_seen_before = \
                    TestDataSampler.sample_from_set(productive_reviewer_set - self.unknown_writer_learned_on_the_fly_id_set)
            # endwhile
        # endif
...
def main_multiprocess(output_root, specification_file_path, debug_mode):
    begin = time.time()
...
if __name__ == '__main__':
    ...
    output_root = "./output_march_04_2022_full"
    specification_file_path = "all_NLT_test_specification_March_04_2022_full.csv"

    # multiprocess
    main_multiprocess(output_root, specification_file_path, debug_mode=False)
